...
METHOD ParseCommand( cCmd ) CLASS TZebraToPdf
   ...
   SWITCH cOpcode
     ...
     CASE "FD"
         ...
         // 1. LIMPEZA DE TAGS HTML/BBCODE DO SISTEMA ERP
...
METHOD DrawBarcodeZPL( cData, cType ) CLASS TZebraToPdf
   ...
   // Inclusão das tipologias com o supressor automático de texto para Códigos 2D
   SWITCH cType
   ...
   // Validação de erro de criação nativa (Evita crash de buffer na LibHaru)
   If hZebra != Nil .AND. hb_zebra_geterror( hZebra ) == 0
      HPDF_Page_SetRGBFill( ::hPage, 0, 0, 0 )
      
      // 1. Calcula a área de ocupação do código de barras em módulos
      ...
      // 2. Trava a altura proporcional para matrizes 2D
      If !lShowText
         nPdfBarHeight := nWidthFactor
      Endif

      // 3. Isola a área gráfica para aplicar a rotação sem afetar o restante do PDF
      HPDF_Page_GSave( ::hPage )

      // 4. Rotação do eixo ancorado no X, Y via Matriz (Concat)
      SWITCH ::cOrientation
         CASE "R" // 90 graus
      ...
      // 5. Desenha o vetor no ponto-âncora (0 relativo) e preenche
      ...
      // 6. Imprime a leitura visual apenas se não for Matriz 2D e o ZPL permitir
      ...
      HPDF_Page_GRestore( ::hPage ) // Libera a rotação
